...
#elif (defined(__AVR__) || defined(__arm__) || defined(__ARDUINO_ARC__) || defined(ESP8266)) && \
        !defined(__linux__)  // Store the DMP firmware in PROGMEM on older AVR (ATmega) platforms
...
const int DMP_Activity_Recognition_Byte_Ordering[icm_20948_DMP_Activity_Recognition_Bytes] = {0, 1, 5, 4, 3, 2};
const int DMP_Secondary_On_Off_Byte_Ordering[icm_20948_DMP_Secondary_On_Off_Bytes] = {1, 0};
...
ICM_20948_Status_e ICM_20948_i2c_controller_periph4_txn(
    ...
    while (nByte < len) {
        ...
        // Kick off txn
        retval = ICM_20948_set_bank(pdev, 3);
        retval = ICM_20948_execute_w(
                pdev, AGB3_REG_I2C_PERIPH4_CTRL, (uint8_t*) &ctrl, sizeof(ICM_20948_I2C_PERIPH4_CTRL_t));
        if (retval != ICM_20948_Stat_Ok) {
            return retval;
        }
...
ICM_20948_Status_e ICM_20948_int_enable(
    ...
    if (read != NULL) {  // If read pointer is not NULL then read the registers (if write is not NULL then this should
                         // read back the results of write into read)
...
ICM_20948_Status_e ICM_20948_set_full_scale(
    ...
    if (sensors & ICM_20948_Internal_Acc) {
        ...
        retval = static_cast<ICM_20948_Status_e>(ICM_20948_set_bank(pdev, 2));
...
ICM_20948_Status_e inv_icm20948_write_mems(
    ...
    while (bytesWritten < length) {
        ...
        /* Sets the starting read or write address for the selected memory, inside of the selected page (see MEM_SEL
           Register). Contents are changed after read or write of the selected memory. This register must be written
           prior to each access to initialize the register to the proper starting address. The address will auto
           increment during burst transactions.  Two consecutive bursts without re-initializing the start address would
           skip one address. */
...
ICM_20948_Status_e inv_icm20948_set_dmp_sensor_period(
    ...
    // In order to set an ODR for a given sensor data, write 2-byte value to DMP using key defined above for a
    // particular sensor. Setting value can be calculated as follows: Value = (DMP running rate (225Hz) / ODR ) - 1 E.g.
    // For a 25Hz ODR rate, value= (225/25) -1 = 8.
...
ICM_20948_Status_e inv_icm20948_enable_dmp_sensor(
    ...
    uint8_t androidSensor =
            sensor_type_2_android_sensor(sensor);  // Convert sensor from enum inv_icm20948_sensor to Android numbering
    ...
    // Convert the Android sensor number into a bitmask and set or clear that bit in _enabled_Android_0 /
    // _enabled_Android_1
    ...
    // Now we know androidSensor is valid, reconstruct the value for DATA_OUT_CTL1 from _enabled_Android_0 and
    // _enabled_Android_0
    delta = 0;  // Clear delta
    for (int i = 0; i < 32; i++) {
        ...
        if (((androidSensorAsBitMask & INV_NEEDS_ACCEL_MASK) > 0) ||
                ((androidSensorAsBitMask & INV_NEEDS_ACCEL_MASK1) > 0)) {
        ...
        if (((androidSensorAsBitMask & INV_NEEDS_GYRO_MASK) > 0) ||
                ((androidSensorAsBitMask & INV_NEEDS_GYRO_MASK1) > 0)) {
        ...
        if (((androidSensorAsBitMask & INV_NEEDS_COMPASS_MASK) > 0) ||
                ((androidSensorAsBitMask & INV_NEEDS_COMPASS_MASK1) > 0)) {
    ...
    // Check if Accel, Gyro/Gyro_Calibr or Compass_Calibr/Quat9/GeoMag/Compass are to be enabled. If they are then we
    // need to request the accuracy data via header2.
    uint16_t delta2 = 0;
    ...
    if (((delta & DMP_Data_Output_Control_1_Step_Detector) > 0) ||
            ((delta & DMP_Data_Output_Control_1_Step_Ind_0) > 0) ||
            ((delta & DMP_Data_Output_Control_1_Step_Ind_1) > 0) ||
            ((delta & DMP_Data_Output_Control_1_Step_Ind_2) > 0)) {
...
ICM_20948_Status_e inv_icm20948_enable_dmp_sensor_int(
    ...
    // Convert the Android sensor number into a bitmask and set or clear that bit in _enabled_Android_intr_0 /
    ...
    // Now we know androidSensor is valid, reconstruct the value for DATA_INTR_CTL from _enabled_Android_intr_0 and
    // _enabled_Android_intr_0
    delta = 0;  // Clear delta
    for (int i = 0; i < 32; i++) {
        androidSensorAsBitMask = 1L << i;
        if ((pdev->_enabled_Android_intr_0 & androidSensorAsBitMask) >
                0)  // Check if the Android sensor (0-31) interrupt is enabled
        {
            delta |= inv_androidSensor_to_control_bits[i];  // If it is, or the required bits into delta
        }
        if ((pdev->_enabled_Android_intr_1 & androidSensorAsBitMask) >
                0)  // Check if the Android sensor (32-) interrupt is enabled
        {
            delta |= inv_androidSensor_to_control_bits[i + 32];  // If it is, or the required bits into delta
        }
    }
...
ICM_20948_Status_e inv_icm20948_read_dmp_data(ICM_20948_Device_t* pdev, icm_20948_DMP_data_t* data) {
    ...
    if ((data->header & DMP_header_bitmap_Accel) > 0)  // case DMP_header_bitmap_Accel:
    {
        ...
        for (int i = 0; i < icm_20948_DMP_Raw_Accel_Bytes; i++) {
            data->Raw_Accel.Bytes[DMP_PQuat6_Byte_Ordering[i]] =
                    fifoBytes[i];  // Correct the byte order (map big endian to little endian)
        }
    ...
    if ((data->header & DMP_header_bitmap_Gyro) > 0)  // case DMP_header_bitmap_Gyro:
    {
        if (fifo_count < (icm_20948_DMP_Raw_Gyro_Bytes +
                                 icm_20948_DMP_Gyro_Bias_Bytes))  // Check if we need to read the FIFO count again
        {
            result = ICM_20948_get_FIFO_count(pdev, &fifo_count);
            if (result != ICM_20948_Stat_Ok)
                return result;
        ...
        if (fifo_count < (icm_20948_DMP_Raw_Gyro_Bytes + icm_20948_DMP_Gyro_Bias_Bytes))
            return ICM_20948_Stat_FIFOIncompleteData;  // Bail if not enough data is available
        result = ICM_20948_read_FIFO(
                pdev, &fifoBytes[0], (icm_20948_DMP_Raw_Gyro_Bytes + icm_20948_DMP_Gyro_Bias_Bytes));
        if (result != ICM_20948_Stat_Ok)
            return result;
        for (int i = 0; i < (icm_20948_DMP_Raw_Gyro_Bytes + icm_20948_DMP_Gyro_Bias_Bytes); i++) {
            data->Raw_Gyro.Bytes[DMP_Raw_Gyro_Byte_Ordering[i]] =
                    fifoBytes[i];  // Correct the byte order (map big endian to little endian)
        }
    ...
    if ((data->header & DMP_header_bitmap_Compass) > 0)  // case DMP_header_bitmap_Compass:
    {
        ...
        for (int i = 0; i < icm_20948_DMP_Compass_Bytes; i++) {
            data->Compass.Bytes[DMP_PQuat6_Byte_Ordering[i]] =
                    fifoBytes[i];  // Correct the byte order (map big endian to little endian)
        }
    ...
    if ((data->header & DMP_header_bitmap_Quat6) > 0)  // case DMP_header_bitmap_Quat6:
    {
        ...
        for (int i = 0; i < icm_20948_DMP_Quat6_Bytes; i++) {
            data->Quat6.Bytes[DMP_Quat6_Byte_Ordering[i]] =
                    fifoBytes[i];  // Correct the byte order (map big endian to little endian)
        }
    ...
    if ((data->header & DMP_header_bitmap_Quat9) > 0)  // case DMP_header_bitmap_Quat9:
    {
        ...
        for (int i = 0; i < icm_20948_DMP_Quat9_Bytes; i++) {
            data->Quat9.Bytes[DMP_Quat9_Byte_Ordering[i]] =
                    fifoBytes[i];  // Correct the byte order (map big endian to little endian)
        }
    ...
    if ((data->header & DMP_header_bitmap_PQuat6) > 0)  // case DMP_header_bitmap_PQuat6:
    {
        ...
        for (int i = 0; i < icm_20948_DMP_PQuat6_Bytes; i++) {
            data->PQuat6.Bytes[DMP_PQuat6_Byte_Ordering[i]] =
                    fifoBytes[i];  // Correct the byte order (map big endian to little endian)
        }
    ...
    if ((data->header & DMP_header_bitmap_Geomag) > 0)  // case DMP_header_bitmap_Geomag:
    {
        ...
        for (int i = 0; i < icm_20948_DMP_Geomag_Bytes; i++) {
            data->Geomag.Bytes[DMP_Quat9_Byte_Ordering[i]] =
                    fifoBytes[i];  // Correct the byte order (map big endian to little endian)
        }
    ...
    if ((data->header & DMP_header_bitmap_Gyro_Calibr) > 0)  // case DMP_header_bitmap_Gyro_Calibr:
    {
        ...
        // So, we'll skip this...:
        /*
                            if (fifo_count < icm_20948_DMP_Gyro_Calibr_Bytes) // Check if we need to read the FIFO count
           again
                            {
                                            result = ICM_20948_get_FIFO_count(pdev, &fifo_count);
                                            if (result != ICM_20948_Stat_Ok)
                            ...
                            if (fifo_count < icm_20948_DMP_Gyro_Calibr_Bytes)
                                            return ICM_20948_Stat_FIFOIncompleteData; // Bail if not enough data is
           available result = ICM_20948_read_FIFO(pdev, &fifoBytes[0], icm_20948_DMP_Gyro_Calibr_Bytes); if (result !=
           ICM_20948_Stat_Ok) return result; for (int i = 0; i < icm_20948_DMP_Gyro_Calibr_Bytes; i++)
                            {
                                            data->Gyro_Calibr.Bytes[DMP_Quat6_Byte_Ordering[i]] = fifoBytes[i]; //
           Correct the byte order (map big endian to little endian)
                            }
    ...
    if ((data->header & DMP_header_bitmap_Compass_Calibr) > 0)  // case DMP_header_bitmap_Compass_Calibr:
    {
        ...
        for (int i = 0; i < icm_20948_DMP_Compass_Calibr_Bytes; i++) {
            data->Compass_Calibr.Bytes[DMP_Quat6_Byte_Ordering[i]] =
                    fifoBytes[i];  // Correct the byte order (map big endian to little endian)
        }
    ...
    if ((data->header2 & DMP_header2_bitmap_Fsync) > 0)  // case DMP_header2_bitmap_Fsync:
    {
        ...
        /*
                            if (fifo_count < icm_20948_DMP_Fsync_Detection_Bytes) // Check if we need to read the FIFO
           count again
                            {
                                            result = ICM_20948_get_FIFO_count(pdev, &fifo_count);
                                            if (result != ICM_20948_Stat_Ok)
                            ...
                            if (fifo_count < icm_20948_DMP_Fsync_Detection_Bytes)
                                            return ICM_20948_Stat_FIFOIncompleteData; // Bail if not enough data is
           available aShort = 0; result = ICM_20948_read_FIFO(pdev, &fifoBytes[0], icm_20948_DMP_Fsync_Detection_Bytes);
                            if (result != ICM_20948_Stat_Ok)
                                            return result;
...
ICM_20948_Status_e inv_icm20948_set_gyro_sf(ICM_20948_Device_t* pdev, unsigned char div, int gyro_level) {
    ...
    if (pll & 0x80) {
        ResultLL = (MagicConstant * (long long) (1ULL << gyro_level) * (1 + div) / (1270 - (pll & 0x7F)) /
                    MagicConstantScale);
    } else {
