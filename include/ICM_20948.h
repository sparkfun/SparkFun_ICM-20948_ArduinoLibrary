...
class ICM_20948 {
       private:
        // Stream *_debugSerial;     //The stream to send debug messages to if
        // enabled
        bool _printDebug = false;  // Flag to print the serial commands we are
                                   // sending to the Serial port for debug
...
// These lines let the code compile cleanly on as many SAMD boards as possible.
    ...
    !defined(ARDUINO_SAMD51_MICROMOD)  // If it is not a SAMD51 Thing Plus or
                                       // SAMD51 MicroMod
        void enableDebugging(
            Stream &debugPort =
                SerialUSB);  // Given a port to print to, enable debug messages.
#else
        void enableDebugging(
            Stream &debugPort =
                Serial);  // Given a port to print to, enable debug messages.
#endif
#else
        void enableDebugging(
            Stream &debugPort =
...
#endif
#else
        // void enableDebugging(Stream &debugPort = Serial); //Given a port to
        // print to, enable debug messages.
#endif
        ...
        // gfvalvo's flash string helper code:
        ...
        void doDebugPrint(char (*)(const char *), const char *,
                          bool newLine = false);

        void debugPrintf(int i);
        void debugPrintf(float f);

        ICM_20948_AGMT_t agmt;  // Acceleometer, Gyroscope, Magenetometer, and
                                // Temperature data
        ICM_20948_AGMT_t getAGMT(void);  // Updates the agmt field in the object
                                         // and also returns a copy directly
        ...
        const char *statusString(
            ICM_20948_Status_e stat =
                ICM_20948_Stat_NUM);  // Returns a human-readable status
                                      // message. Defaults to status member, but
                                      // prints string for supplied status if
                                      // supplied

        // Device Level
        ICM_20948_Status_e setBank(uint8_t bank);  // Sets the bank
        ICM_20948_Status_e swReset(void);          // Performs a SW reset
        ICM_20948_Status_e sleep(
            bool on = false);  // Set sleep mode for the chip
        ICM_20948_Status_e lowPower(
            bool on = true);  // Set low power mode for the chip
        ICM_20948_Status_e setClockSource(
            ICM_20948_PWR_MGMT_1_CLKSEL_e source);  // Choose clock source
        ICM_20948_Status_e checkID(void);  // Return 'ICM_20948_Stat_Ok' if
                                           // whoami matches ICM_20948_WHOAMI

        bool dataReady(void);     // Returns 'true' if data is ready
        uint8_t getWhoAmI(void);  // Return whoami in out prarmeter
        bool isConnected(void);   // Returns true if communications with the
                                  // device are sucessful

        // Internal Sensor Options
        ICM_20948_Status_e setSampleMode(
            uint8_t sensor_id_bm,
            uint8_t lp_config_cycle_mode);  // Use to set accel, gyro, and I2C
                                            // master into cycled or continuous
                                            // modes
        ICM_20948_Status_e setFullScale(uint8_t sensor_id_bm,
                                        ICM_20948_fss_t fss);
        ICM_20948_Status_e setDLPFcfg(uint8_t sensor_id_bm,
                                      ICM_20948_dlpcfg_t cfg);
        ICM_20948_Status_e enableDLPF(uint8_t sensor_id_bm, bool enable);
        ICM_20948_Status_e setSampleRate(uint8_t sensor_id_bm,
        ...
        ICM_20948_Status_e cfgIntLatch(
            bool latching);  // If not latching then the interrupt is a 50 us
                             // pulse
        ICM_20948_Status_e cfgIntAnyReadToClear(
            bool enabled);  // If enabled, *ANY* read will clear the INT_STATUS
                            // register. So if you have multiple interrupt
                            // sources enabled be sure to read INT_STATUS first
        ICM_20948_Status_e cfgFsyncActiveLow(bool active_low);
        ICM_20948_Status_e cfgFsyncIntMode(
            bool interrupt_mode);  // Can use FSYNC as an interrupt input that
                                   // sets the I2C Master Status register's
                                   // PASS_THROUGH bit
        ...
        ICM_20948_Status_e i2cControllerConfigurePeripheral(
            uint8_t peripheral, uint8_t addr, uint8_t reg, uint8_t len,
            bool Rw = true, bool enable = true, bool data_only = false,
            bool grp = false, bool swap = false, uint8_t dataOut = 0);
        ICM_20948_Status_e i2cControllerPeriph4Transaction(
        ...
        ICM_20948_Status_e i2cMasterConfigureSlave(
            uint8_t peripheral, uint8_t addr, uint8_t reg, uint8_t len,
            bool Rw = true, bool enable = true, bool data_only = false,
            bool grp = false, bool swap = false);
        ICM_20948_Status_e i2cMasterSLV4Transaction(uint8_t addr, uint8_t reg,
                                                    uint8_t *data, uint8_t len,
                                                    bool Rw,
                                                    bool send_reg_addr = true);

        // Used for configuring the Magnetometer
        ICM_20948_Status_e i2cMasterSingleW(uint8_t addr, uint8_t reg,
                                            uint8_t data);
        ...
        ICM_20948_Status_e startupDefault(
            bool minimal =
                false);  // If minimal is true, several startup steps are
                         // skipped. If ICM_20948_USE_DMP is defined, .begin
                         // will call startupDefault with minimal set to true.
        ...
        ICM_20948_Status_e startupMagnetometer(
            bool minimal = false);  // If minimal is true, several startup steps
                                    // are skipped. The mag then needs to be set
                                    // up manually for the DMP.
        ...
        ICM_20948_Status_e setFIFOmode(
            bool snapshot = false);  // Default to Stream (non-Snapshot) mode
        ...
        //  Configuring DMP to output data to FIFO: set DATA_OUT_CTL1,
        //  DATA_OUT_CTL2, DATA_INTR_CTL and MOTION_EVENT_CTL Configuring DMP to
        //  output data at multiple ODRs Configure DATA_RDY_STATUS Configuring
        //  Accel calibration Configuring Compass calibration Configuring Gyro
        //  gain Configuring Accel gain Configure I2C_SLV0 and I2C_SLV1 to:
        //  request mag data from the hidden reserved AK09916 registers; trigger
        //  Single Measurements Configure I2C Master ODR (default to 68.75Hz)

        // To Do:
        //  Additional FIFO output control: FIFO_WATERMARK, BM_BATCH_MASK,
        //  BM_BATCH_CNTR, BM_BATCH_THLD Configuring DMP features:
        //  PED_STD_STEPCTR, PED_STD_TIMECTR Enabling Activity Recognition (BAC)
        //  feature Enabling Significant Motion Detect (SMD) feature Enabling
        //  Tilt Detector feature Enabling Pick Up Gesture feature Enabling
        //  Fsync detection feature Biases: add save and load methods

        ICM_20948_Status_e enableDMP(bool enable = true);
        ICM_20948_Status_e resetDMP(void);
        ICM_20948_Status_e loadDMPFirmware(void);
        ICM_20948_Status_e setDMPstartAddress(
            unsigned short address = DMP_START_ADDRESS);
        ICM_20948_Status_e enableDMPSensor(enum inv_icm20948_sensor sensor,
                                           bool enable = true);
        ICM_20948_Status_e enableDMPSensorInt(enum inv_icm20948_sensor sensor,
                                              bool enable = true);
        ICM_20948_Status_e writeDMPmems(unsigned short reg, unsigned int length,
                                        const unsigned char *data);
        ICM_20948_Status_e readDMPmems(unsigned short reg, unsigned int length,
                                       unsigned char *data);
        ICM_20948_Status_e setDMPODRrate(enum DMP_ODR_Registers odr_reg,
                                         int interval);
        ...
        ICM_20948_Status_e initializeDMP(void)
            __attribute__((weak));  // Combine all of the DMP start-up code in
                                    // one place. Can be overwritten if required
};

// I2C

// Forward declarations of TwoWire and Wire for board/variant combinations that
// don't have a default 'SPI'
// class TwoWire; // Commented by PaulZC 21/2/8 - this was causing compilation
// to fail on the Arduino NANO 33 BLE extern TwoWire Wire; // Commented by PaulZC
// 21/2/8 - this was causing compilation to fail on the Arduino NANO 33 BLE
