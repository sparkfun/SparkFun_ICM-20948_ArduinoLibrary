...
// These lines let the code compile cleanly on as many SAMD boards as possible.
...
#else
  void enableDebugging(Stream &debugPort = Serial); // Given a port to print to,
  ...
  const char *
  statusString(ICM_20948_Status_e stat =
                   ICM_20948_Stat_NUM); // Returns a human-readable status
                                        // message. Defaults to status member,
                                        // but prints string for supplied status
                                        // if supplied
  ...
  ICM_20948_Status_e
  lowPower(bool on = true); // Set low power mode for the chip
  ICM_20948_Status_e
  setClockSource(ICM_20948_PWR_MGMT_1_CLKSEL_e source); // Choose clock source
  ...
  ICM_20948_Status_e
  setSampleMode(uint8_t sensor_id_bm,
                uint8_t lp_config_cycle_mode); // Use to set accel, gyro, and
                                               // I2C master into cycled or
                                               // continuous modes
  ...
  ICM_20948_Status_e cfgIntLatch(bool latching); // If not latching then the
                                                 // interrupt is a 50 us pulse
  ...
  ICM_20948_Status_e
  cfgFsyncIntMode(bool interrupt_mode); // Can use FSYNC as an interrupt input
                                        // that sets the I2C Master Status
                                        // register's PASS_THROUGH bit
  ...
  ICM_20948_Status_e i2cControllerPeriph4Transaction(uint8_t addr, uint8_t reg,
  ...
  ICM_20948_Status_e i2cMasterConfigureSlave(uint8_t peripheral, uint8_t addr,
                                             uint8_t reg, uint8_t len,
                                             bool Rw = true, bool enable = true,
  ...
  ICM_20948_Status_e
  setFIFOmode(bool snapshot = false); // Default to Stream (non-Snapshot) mode
  ...
  ICM_20948_Status_e
  setDMPstartAddress(unsigned short address = DMP_START_ADDRESS);
...
// to fail on the Arduino NANO 33 BLE extern TwoWire Wire; // Commented by
// PaulZC 21/2/8 - this was causing compilation to fail on the Arduino NANO 33
// BLE
