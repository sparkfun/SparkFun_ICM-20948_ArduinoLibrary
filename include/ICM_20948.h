...
// These lines let the code compile cleanly on as many SAMD boards as possible.
...
#if !defined(ARDUINO_SAMD51_THING_PLUS) & !defined(ARDUINO_SAMD51_MICROMOD)  // If it is not a SAMD51 Thing Plus or
                                                                             // SAMD51 MicroMod
    ...
    const char* statusString(ICM_20948_Status_e stat = ICM_20948_Stat_NUM);  // Returns a human-readable status
    ...
    ICM_20948_Status_e setSampleMode(uint8_t sensor_id_bm,
    ...
    ICM_20948_Status_e cfgIntLatch(bool latching);          // If not latching then the interrupt is a 50 us
                                                            // pulse
    ICM_20948_Status_e cfgIntAnyReadToClear(bool enabled);  // If enabled, *ANY* read will clear the INT_STATUS
    ...
    ICM_20948_Status_e cfgFsyncIntMode(bool interrupt_mode);  // Can use FSYNC as an interrupt input that
    ...
    ICM_20948_Status_e i2cControllerConfigurePeripheral(uint8_t peripheral, uint8_t addr, uint8_t reg, uint8_t len,
            bool Rw = true, bool enable = true, bool data_only = false, bool grp = false, bool swap = false,
            uint8_t dataOut = 0);
    ICM_20948_Status_e i2cControllerPeriph4Transaction(
            uint8_t addr, uint8_t reg, uint8_t* data, uint8_t len, bool Rw, bool send_reg_addr = true);
    ...
    ICM_20948_Status_e i2cMasterConfigureSlave(uint8_t peripheral, uint8_t addr, uint8_t reg, uint8_t len,
            bool Rw = true, bool enable = true, bool data_only = false, bool grp = false, bool swap = false);
    ICM_20948_Status_e i2cMasterSLV4Transaction(
            uint8_t addr, uint8_t reg, uint8_t* data, uint8_t len, bool Rw, bool send_reg_addr = true);
    ...
    // Default Setup
    ICM_20948_Status_e startupDefault(bool minimal = false);  // If minimal is true, several startup steps are
    ...
    // Mag specific
    ICM_20948_Status_e startupMagnetometer(bool minimal = false);  // If minimal is true, several startup steps
    ...
    ICM_20948_Status_e initializeDMP(void) __attribute__((weak));  // Combine all of the DMP start-up code in
